...
function TreatmentCard({t,idx,onStop,onDel}){
  ...
  return <div style={{borderRadius:14,padding:"13px 16px",marginBottom:10,borderLeft:"4px solid "+color,background:topical?"#eaf3f9":"#e3f7f2"}}>
    ...
    {t.bodyAreas&&t.bodyAreas.length>0&&<div style={{fontSize:"0.76rem",color:"var(--text-body)",marginBottom:3}}>Areas: {t.bodyAreas.join(", ")}</div>}
...
function TreatmentsTab(){
  // Storage-backed — FW mutations persist to localStorage and re-render via FW.onDataChange
  const treatments=FWd.treatments;
  ...
  const [drug,setDrug]=React.useState(topicalDrugs[0]);
  ...
  const openForm=ty=>{setForm(ty);setDrug((ty==="topical"?topicalDrugs:biologicDrugs)[0]);setAreas([]);setStart(today);setStop("");setNotes("")};
  const stopT=t=>FWd.stopTreatment(t.id);
  const del=t=>{if(confirm("Delete this treatment record?"))FWd.deleteTreatment(t.id)};
  // Save — same validation + record shape as js/v2/treatments.js; the class is
  // derived from the chosen drug (v3's flat drug list has no class dropdowns)
  const saveT=()=>{
    if(!start){alert("Please enter a start date.");return}
    const isTopical=form==="topical";
    FWd.addTreatment({id:Date.now().toString(),type:form,
      potencyClass:isTopical?FWd.classForDrug("topical",drug):null,
      biologicClass:!isTopical?FWd.classForDrug("biologic",drug):null,
      drug,bodyAreas:isTopical?[...areas]:[],startDate:start,stopDate:stop||null,notes:notes.trim()});
    setForm(null);
  };
  return <div>
    {!form&&<div style={{display:"flex",gap:10,marginBottom:18}}>
      <Button variant="add" onClick={()=>openForm("topical")}>＋ Topical steroid</Button>
      <Button variant="add" onClick={()=>openForm("biologic")}>＋ Biologic</Button>
    </div>}
    {form&&<div style={{background:"var(--surface-tint-primary)",border:"1.5px solid var(--border-default)",borderRadius:16,padding:"18px 20px",marginBottom:16}}>
      <div style={{display:"flex",gap:16,marginBottom:14}}>
        {["topical","biologic"].map(ty=><label key={ty} style={{display:"flex",alignItems:"center",gap:6,fontSize:"0.85rem",fontWeight:600,color:"var(--text-body)",cursor:"pointer"}}>
          <input type="radio" checked={form===ty} onChange={()=>{setForm(ty);setDrug((ty==="topical"?topicalDrugs:biologicDrugs)[0])}} style={{accentColor:"var(--brand-blue)"}}/>{ty==="topical"?"Topical steroid":"Biologic"}</label>)}
      </div>
      <div style={{display:"flex",gap:10,marginBottom:12,flexWrap:"wrap"}}>
        <Field label="Drug"><Select options={form==="topical"?topicalDrugs:biologicDrugs} value={drug} onChange={e=>setDrug(e.target.value)}/></Field>
      </div>
      ...
      <div style={{display:"flex",gap:10,marginBottom:12,flexWrap:"wrap"}}>
        <Field label="Start date"><TextInput type="date" value={start} max={today} onChange={e=>setStart(e.target.value)}/></Field>
        <Field label="Stop date (leave blank if ongoing)"><TextInput type="date" value={stop} onChange={e=>setStop(e.target.value)}/></Field>
      </div>
      <div style={{display:"flex",gap:10,marginBottom:12}}>
        <Field label="Notes (optional)"><TextInput type="text" placeholder="Dose, frequency, side effects…" value={notes} onChange={e=>setNotes(e.target.value)}/></Field>
      </div>
      <div style={{display:"flex",gap:10,marginTop:14}}>
        <Button variant="secondary" style={{padding:"9px 20px",fontSize:"0.88rem"}} onClick={()=>setForm(null)}>Cancel</Button>
        <Button variant="primary" style={{flex:1,padding:"9px 0",fontSize:"0.88rem",background:"var(--brand-blue)",boxShadow:"none"}} onClick={saveT}>Save treatment</Button>
      </div>
    </div>}
    ...
    {active.map(t=><TreatmentCard key={t.id} t={t} idx={treatments.indexOf(t)} onStop={()=>stopT(t)} onDel={()=>del(t)}/>)}
    {past.length>0&&head("Past")}
    {past.map(t=><TreatmentCard key={t.id} t={t} idx={treatments.indexOf(t)} onStop={()=>stopT(t)} onDel={()=>del(t)}/>)}
  </div>;
}
