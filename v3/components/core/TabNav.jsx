...
export function TabNav({tabs,active,onChange}){
  ...
  return <nav ref={navRef} style={{display:"flex",gap:5,background:"var(--surface-rail)",borderRadius:14,padding:5,position:"relative"}}>
    ...
    {tabs.map(t=><button key={t} ref={el=>refs.current[t]=el} onClick={()=>onChange&&onChange(t)}
      style={{position:"relative",zIndex:1,flex:1,padding:"11px 2px",border:"none",borderRadius:10,background:"transparent",color:t===active?"var(--brand-blue-deep)":"var(--text-muted)",fontSize:"0.8rem",fontWeight:600,cursor:"pointer",fontFamily:"inherit",transition:"color .2s"}}
