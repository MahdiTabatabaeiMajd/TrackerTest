...
- **Layout:** single centered white card, CONSTANT width 700px (via a definite-width #root + `scrollbar-gutter:stable` — never let the card shrink to content), radius 24px, `overflow:clip` (not hidden — keeps the sticky Save bar working), big soft shadow, 5px gradient accent bar pinned to its top edge, footer credit "© 2026 INIA Biosciences · Built by Mahdi Tabatabaei". Inside: tinted section panels (radius 18px, 1px `--border-soft` border) containing white sub-cards (radius 14px, no shadow). Tab panels have minHeight 640 so the form doesn't resize between tabs. Form actions live in a sticky frosted bar at the card bottom. Touch targets ≥40px (steppers, toggles, chips, tab nav). 2-column grids collapse to 1 at ≤520px.
...
## Index
- `styles.css` → `tokens/` (colors, typography, spacing, effects, base)
